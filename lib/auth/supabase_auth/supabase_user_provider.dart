...
class StylistSupabaseUser extends BaseAuthUser {
  StylistSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> stylistSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = StylistSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
